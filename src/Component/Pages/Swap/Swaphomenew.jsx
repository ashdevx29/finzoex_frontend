...
function Home() {
  ...
  const [revfromCurrency, setrevfromCurrency, revfromref] = useState("BTC");
  const [revtoCurrency, setrevtoCurrency, revtoref] = useState("USDT");
  const [revappendFromData, setrevappendFromData,revappendFromDataref] = useState("");
  const [revappendToData, setrevappendFToData,revappendToDataref] = useState("");

  const [revallCurrencyFiat, setrevfromCurrencyRef, revfromCurrencyRef] = useState([]);
  const [revtoCurrencyRefs, setrevToCurrencyRef, revtoCurrencyRef] = useState([]);

  const [defaultFrom, setdefaultFrom, defaultFromref] = useState("619948553cda890ff0256d3c");
  const [defaultTo, setdefaultTo, defaultToref] = useState("623ce3a0cc89bdb6a5766121");

  const [revdefaultFrom, setrevdefaultFrom, revdefaultFromref] = useState("619948553cda890ff0256d3c");
  const [revdefaultTo, setrevdefaultTo, revdefaultToref] = useState("623ce3a0cc89bdb6a5766121");
  const [revshow,setRevshow,revshowref] = useState(false);
  ...
  const swapAmount = async () => {
    try {
      if (
        ...
        if (appendFromDataref.current.currencySymbol != appendToDataref.current.currencySymbol) {
  ...
  const maxBalance = async () => {
    try {
      setmaxValue(true);
      var fee =
        (+appendFromDataref.current.currencyBalance * +appendFromDataref.current.swapFee) / 100;
  ...
  const getUserbalance = async () => {
    ...
    if (resp.status) {
      ...
      var default_from = (resp.data[0].currid != null) ? resp.data[0].currid.toString() : defaultFromref.current;
      ...
      var default_to = (resp.data[2].currid != null) ? resp.data[2].currid.toString() : defaultToref.current;
      setdefaultTo(default_to)
      swapPrice();
    } else {
    }
  };
  ...
  const Swap = () => {
    var Tempcurrency;
    var Temptotal;
    // console.log(appendFromDataref.current.currencySymbol,"fromCurrencyRef");
    // console.log(fromAmountref.current,"fromAmountref");
    // console.log(appendToDataref.current.current,"toCurrencyRef");
    // console.log(toAmount,"fromAmountref");

    setRevshow(true);

    console.log("fromCurrencyRef.current)",fromCurrencyRef.current)
    console.log("toCurrencyRef.current)",toCurrencyRef.current)
    setrevfromCurrencyRef(fromCurrencyRef.current);
    setrevToCurrencyRef(toCurrencyRef.current);

    setrevappendFromData(appendFromDataref.current);
    setrevfromCurrency(fromref.current);

    setrevappendFToData(appendToDataref.current);
    setrevtoCurrency(toref.current);

    // setrevdefaultFrom(defaultFromref.current);
    // setrevdefaultTo(defaultToref.current);

    
    setfromCurrencyRef(revtoCurrencyRef.current);

    setToCurrencyRef(revfromCurrencyRef.current);

    console.log("fromCurrencyRef.current)",fromCurrencyRef.current)
    console.log("toCurrencyRef.current)",toCurrencyRef.current)

    setappendFromData(revappendToDataref.current);
    console.log("appendFrom===",appendFromDataref.current)
    setFromcurrencyImage(tocurrencyImage);
    setfromCurrency(revtoref.current);
    console.log("setfromCurrency===",fromref.current)
   //setdefaultFrom(revdefaultToref.current);
    

    settoCurrency(revfromref.current);
    console.log("settoCurrency===",toref.current)
    setappendFToData(revappendFromDataref.current);
    console.log("setappendFToData===",appendToDataref.current)
    setTocurrencyImage(fromcurrencyImage);
    //setdefaultTo(revdefaultFromref.current);
    swapPrice();
  };
                                    ...
                                    <div className="account_setting">
                                      <div className="form-group groow-1_widut" style={{ position: "relative", zIndex: 100 }}>
                                        ...
                                        <Dropdown
                                          ...
                                          defaultValue={defaultFromref.current}
                                          search
                                        />
                                      </div>
                                    ...
                                    <div className="account_setting">
                                      <div className="form-group groow-1_widut" style={{ position: "relative", zIndex: 90 }}>
                                        ...
                                        <Dropdown
                                          ...
                                          className="text_memu swapmenui"
                                          defaultValue={defaultToref.current}
                                          search
                                        />
                                      </div>
                                      <div className="form-group groow-1_widut ">
